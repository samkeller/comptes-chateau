...
describe("AccountLineService.save - transfer groups", () => {
    let nextId: number;
    ...
    const accountLineRepo = {
        ...
        findBy: vi.fn(async (where: { transferGroupId?: string; id?: number[] | { _type: string; _value: number[] } }) => {
            const ids = Array.isArray(where.id) ? where.id : where.id?._value;
            return storedLines.filter((entry) => {
                const groupMatches = where.transferGroupId === undefined || entry.transferGroupId === where.transferGroupId;
                const idMatches = ids === undefined || ids.includes(entry.id);
    ...
    it("saves generated account lines through saveAll with normalized dates", async () => {
        const service = new AccountLineService();

        const savedLines = await service.saveAll([{
            label: "Courses",
        ...
        expect(savedLines[0].id).toBe(1);
        expect(savedLines[0].dateOperation).toBeInstanceOf(Date);
    });

    it("rejects inconsistent generated account lines before persisting a saveAll batch", async () => {
        const service = new AccountLineService();
    ...
    it("keeps persisted check state when saveAll updates unrelated fields", async () => {
        storedLines = [
            {
                id: 7,
        ...
        const service = new AccountLineService();
        const [savedLine] = await service.saveAll([{ id: 7, label: "Renommée" }]);

        expect(savedLine.label).toBe("Renommée");
        expect(savedLine.isChecked).toBe(true);
    });
    ...
    it("propagates amounts, dateOperation and check state to the sibling on transfer edit", async () => {
        ...
        expect(formatLocalDate(sibling?.dateOperation as Date)).toBe("2026-03-22");
...
describe("AccountLineService.delete - transfer groups", () => {
    let nextId: number;
    let storedLines: StoredLine[];
...
describe("AccountLineService.duplicateLine - transfer groups", () => {
    let nextId: number;
    let storedLines: StoredLine[];
...
describe("AccountLineService.checkBatch - transfer groups", () => {
    let storedLines: StoredLine[];
    let storedImportedOperations: Array<{
        id: number;
        accountId: number;
