...
export default class AccountLineService {
    ...
    /**
     * Normalise les dates d'une ligne et valide les invariants métier/DB.
     */
    ...
    private async saveAccountLine(accountLine: Partial<AccountLine>): Promise<AccountLine> {
    ...
    async saveAll(accountLines: Partial<AccountLine>[]): Promise<AccountLine[]> {
    ...
    async save(line: SaveOperationPayload, accountId: number, userId: number): Promise<AccountLine> {
        return AppDataSource.transaction(async (manager) => {
            ...
            if (!targetAccount) {
                ...
                const normalizedPrimary = await accountLineService.saveAccountLine({
                    ...primaryLine,
                    targetAccount: null,
            ...
            const savedPrimary = await accountLineService.saveAccountLine(primaryLine);
            ...
            if (sibling) {
                if (hasTargetChanged) {
                    ...
                    await accountLineService.saveAccountLine({ ...mirrorPayload, id: sibling.id });
                } else {
                    await accountLineService.saveAccountLine({
                        id: sibling.id,
                ...
                await accountLineService.saveAccountLine(mirrorPayload);
            }
    ...
    async duplicateLine(accountId: number, lineId: number) {
        return AppDataSource.transaction(async (manager) => {
            ...
            const savedLine = await accountLineService.saveAccountLine(newLine);
